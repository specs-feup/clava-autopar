...
export default function SetArrayAccessOpenMPscoping($ForStmt: Loop) {
    ...
    for (const arrayAccessObj of LoopOmpAttributes[loopindex].varAccess)
        if (
            ...
            if (arrayAccessObj.name === undefined) {
                throw new Error("arrayAccessObj.name is undefined");
            }
            if (
    ...
    if (arrayAccesslist.length === 0) return;
    ...
    for (const depObj of LoopOmpAttributes[loopindex].PetitFoundDependency)
        if (
            ...
            arrayAccessNamelist.indexOf(depObj.varName) !== -1 &&
            depObj.cannotbesolved === false
        ) {
            ...
            if (arrayAccessObj.usedInClause === true) {
                arrayAccessNamelist.splice(
                    arrayAccessNamelist.indexOf(depObj.varName),
                    1
                );
                continue;
            }
            ...
            if (
                ...
                if (
                    depObj.IsdependentCurrentloop_src !== undefined &&
                    depObj.IsdependentInnerloop_src !== undefined &&
                    depObj.IsdependentOuterloop_src
                ) {
                    const o = FindReductionArrays(
                        $ForStmt,
                        depObj.varName,
                        depObj.varref_line_src,
                ...
                throw new Error("depObj.Isdependent is undefined");
            }
        }
    ...
    for (const arrayAccessObj of arrayAccesslist)
        if (arrayAccessObj.usedInClause === false) {
            arrayAccessObj.usedInClause = true;
            for (const depObj of LoopOmpAttributes[loopindex]
                .PetitFoundDependency)
                if (depObj.varName === arrayAccessObj.name)
                    arrayAccessObj.usedInClause =
                        arrayAccessObj.usedInClause &&
                        depObj.canbeignored &&
                        depObj.cannotbesolved === false;
            if (arrayAccessObj.name !== undefined) {
        ...
        for (const depObj of LoopOmpAttributes[loopindex].PetitFoundDependency)
            if (
                ...
                if (
                    ...
                    LoopOmpAttributes[loopindex].Reduction_listVars?.indexOf(
                        arrayAccessObj.name
                    ) === -1
                ) {
                    LoopOmpAttributes[loopindex].firstprivateVars.push(
    ...
    for (const arrayAccessObj of LoopOmpAttributes[loopindex].varAccess)
        if (
            ...
            Add_msgError(
                LoopOmpAttributes,
                $ForStmt,
                "unsolved dependency for arrayAccess " +
                    arrayAccessObj.name +
                    "\t use : " +
                    arrayAccessObj.use
            );
