 ...
 *
 **************************************************************/
export default function FindReductionArrays(
  $ForStmt: Loop,
  candidateArrayName: string,
  exprline: number,
  ...
  const candidateArray = SearchStruct(LoopOmpAttributes[loopindex].varAccess, {
    usedInClause: false,
    name: candidateArrayName,
  })[0];

  const count = candidateArray.useT.match(/RW/g) || [];

  if (count.length > 1 || count.length === 0) return;

  // check for similar dependency of all occurrence for candidateArray
  for (let i = 1; i < candidateArray.varUsage.length; i++)
    if (
      candidateArray.varUsage[i].IsdependentCurrentloop !==
        candidateArray.varUsage[0].IsdependentCurrentloop ||
      candidateArray.varUsage[i].IsdependentInnerloop !==
        candidateArray.varUsage[0].IsdependentInnerloop ||
      candidateArray.varUsage[i].IsdependentOuterloop !==
        candidateArray.varUsage[0].IsdependentOuterloop
    )
      return;

  for (const $expr of Query.searchFrom($ForStmt.body, Expression, {
    line: exprline,
  })) {
    reduction = retReductionOpArray(
      $expr,
      candidateArray,
  ...
  if (reduction.length > 0) {
    LoopOmpAttributes[loopindex].Reduction =
      LoopOmpAttributes[loopindex].Reduction?.concat(reduction);
 ...
 *
 **************************************************************/
function retReductionOpArray(
  $expr: Expression,
  candidateVar: VarAccess,
  isdependentInnerloop: boolean,
) {
  ...
  for (let j = 0; j < exprvarrefset.length; j++) {
    ...
    if (
      exprvarrefset[j].name === candidateVar.name &&
      exprvarrefset[j].instanceOf("varref")
    ) {
      if (exprvarrefset[j].useExpr.use.indexOf("write") !== -1)
        candidateVarUse = exprvarrefset[j].useExpr.code;

      if (
        exprvarrefset[j].getAncestor("expression") !== undefined &&
        exprvarrefset[j].getAncestor("expression").astName === "ParenExpr"
      ) {
        candidateVarOp = [];
      ...
      if ((exprvarrefset[j].getAncestor("unaryOp") as UnaryOp) !== undefined)
        ...
        op =
          (exprvarrefset[j].getAncestor("binaryOp") as BinaryOp).kind +
          // TODO: Is this dead code?
          ((exprvarrefset[j].getAncestor("binaryOp") as BinaryOp).kind !==
            "assign" &&
          (exprvarrefset[j].getAncestor("binaryOp") as BinaryOp)
            .isAssignment === true
            ? ""
            : "");
      if (
        op === "sub" &&
        typeof (exprvarrefset[j].getAncestor("binaryOp") as BinaryOp).right
          .vardecl !== "undefined" &&
        (exprvarrefset[j].getAncestor("binaryOp") as BinaryOp).right.vardecl
  ...
  if (candidateVar.ArraySize === null) return reduction;

  if (isdependentInnerloop === true)
    arraysizeStr =
      candidateVar.name + candidateVar.ArraySize.replaceAll("\\[", "[:");
